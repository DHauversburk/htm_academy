...
import { type WorkOrder as WorkOrderType } from './game/types';
...
function App() {
  ...
  const [currentWO, setCurrentWO] = useState<WorkOrderType | null>(null);
  ...
  useEffect(() => {
    ...
    const handleOpenWO = (data: WorkOrderType) => {
      setCurrentWO(data);
    ...
    const handleStartRepair = (data: { details: WorkOrderType }) => {
      // Close UI overlays
      setIsWorkOrderOpen(false);
    ...
    const handleShowToast = (data: string | { message: string, type: 'success' | 'error' }) => {
