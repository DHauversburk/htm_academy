            ...
            {workOrders.map((order) => {
                ...
                return (
                    <motion.div
                        ...
                        onClick={() => !isClosed && handleSelectOrder(order)}
                        className={clsx(
                            "p-4 rounded-xl border border-slate-700 shadow-xl transition-colors backdrop-blur-md",
                            isActive ? "bg-blue-900/80 border-blue-500" : "bg-slate-800/90",
                            isClosed ? "opacity-60 cursor-not-allowed" : "cursor-pointer",
                            !isActive && !isClosed && "hover:bg-slate-800"
                        )}
                    >
